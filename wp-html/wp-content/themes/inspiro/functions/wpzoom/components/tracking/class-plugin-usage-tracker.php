	...
	class Plugin_Usage_Tracker {
		...
		public function schedule_weekly_event( $schedules ) {
			$schedules['weekly'] = array(
				'interval'	=> 604800,
				'display'		=> __( 'Once Weekly', 'wpzoom' )
			);
			$schedules['monthly'] = array(
				'interval'	=> 2635200,
				'display'		=> __( 'Once Monthly', 'wpzoom' )
			);
			return $schedules;
		}
		...
		public function get_data() {
			...
			if( empty( $plugin ) ) {
				...
				$body['message'] .= __( 'We can\'t detect any product information. This is most probably because you have not included the code snippet.', 'wpzoom' );
		...
		public function optin_notice() {
			...
			if ( $is_local ) {
				...
				if( $this->marketing != 1 ) {
					...
					$notice_text = sprintf(
						__( 'Thank you for installing our WPZOOM %1$s. We would like to track its usage on your site. We don\'t record any sensitive data, only information regarding the WordPress environment and %1$s settings (WordPress version, PHP version, etc.), which we will use to help us make improvements to the %1$s. Tracking is completely optional.', 'wpzoom' ),
						$this->what_am_i
					);
				} else {
					// If we have option 1 for marketing, we include reference to sending product information here
					$notice_text = sprintf(
						__( 'Thank you for installing our %1$s. We\'d like your permission to track its usage on your site and subscribe you to our newsletter. We won\'t record any sensitive data, only information regarding the WordPress environment and %1$s settings, which we will use to help us make improvements to the %1$s. Tracking is completely optional.', 'wpzoom' ),
						$this->what_am_i
					);
				}
				...
				<div class="notice notice-info updated put-dismiss-notice">
					...
					<p>
						<a href="<?php echo esc_url( $url_yes ); ?>" class="button-secondary"><?php _e( 'Allow', 'wpzoom' ); ?></a>
						<a href="<?php echo esc_url( $url_no ); ?>" class="button-secondary"><?php _e( 'Do Not Allow', 'wpzoom' ); ?></a>
					</p>
				</div>
			<?php
			}
		...
		public function marketing_notice() {
			...
			if( isset( $_GET['marketing_optin'] ) ) {
				...
				$marketing_text = sprintf(
					__( 'Thank you for opting in to tracking. Would you like to receive occasional news about this %s, including details of new features and special offers?', 'wpzoom' ),
					$this->what_am_i
				);
				...
				<div class="notice notice-info updated put-dismiss-notice">
					...
					<p>
						<a href="<?php echo esc_url( $url_yes ); ?>" data-putnotice="yes" class="button-secondary"><?php _e( 'Yes, Please', 'wpzoom' ); ?></a>
						<a href="<?php echo esc_url( $url_no ); ?>" data-putnotice="no" class="button-secondary"><?php _e( 'No, Thank You', 'wpzoom' ); ?></a>
					</p>
				</div>
				<?php }
		}
		...
		public function form_default_text() {
			$form = array();
			$form['heading'] = __( 'Sorry to see you go', 'wpzoom' );
			$form['body'] = __( 'Before you deactivate the plugin, would you quickly give us your reason for doing so?', 'wpzoom' );
			$form['options'] = array(
				__( 'Set up is too difficult', 'wpzoom' ),
				__( 'Lack of documentation', 'wpzoom' ),
				__( 'Not the features I wanted', 'wpzoom' ),
				__( 'Found a better plugin', 'wpzoom' ),
				__( 'Installed by mistake', 'wpzoom' ),
				__( 'Only required temporarily', 'wpzoom' ),
				__( 'Didn\'t work', 'wpzoom' )
			);
			$form['details'] = __( 'Details (optional)', 'wpzoom' );
			return $form;
		}
		...
		public function goodbye_ajax() {
			...
			$html .= '<p class="deactivating-spinner"><span class="spinner"></span> ' . __( 'Submitting form', 'wpzoom' ) . '</p>';
			?>
			...
			<script>
				jQuery(document).ready(function($){
					$("#put-goodbye-link-<?php echo esc_attr( $this->plugin_name ); ?>").on("click",function(){
						...
						$("#put-goodbye-form-<?php echo esc_attr( $this->plugin_name ); ?>").html( '<?php echo $html; ?>' + '<div class="put-goodbye-form-footer"><p><a id="put-submit-form" class="button primary" href="#"><?php _e( 'Submit and Deactivate', 'wpzoom' ); ?></a>&nbsp;<a class="secondary button" href="'+url+'"><?php _e( 'Just Deactivate', 'wpzoom' ); ?></a></p></div>');
